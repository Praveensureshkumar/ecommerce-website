function updatePrices() {
  ...
  document.querySelectorAll('.card.cart-item').forEach(card => {
      ...
      const quantity = parseFloat(card.querySelector('input.quantity').value);
      ...
      // Delivery: ₹50 per product if productTotal <= 500
      if (productTotal <= 500) {
          deliveryCharges += 50;
      }

      // Calculate maximum delivery if no free delivery
      maxDelivery += 50;
  });
  ...
  // Stylish delivery charge display
